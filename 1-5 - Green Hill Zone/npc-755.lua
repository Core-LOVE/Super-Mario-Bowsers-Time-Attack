...
function buzzbomberNPC.onTickEndNPC(v)
	...
	if not v:mem(0x128,FIELD_BOOL) then
		...
		data.ftimer = data.ftimer + 1
			if data.ftimer == 10 then
				if v.direction == -1 then
					local fire = NPC.spawn(754,v.x+15, v.y+70,player.section)
					fire.speedY = 2
					fire.speedX = -4
				elseif v.direction == 1 then
					local fire2 = NPC.spawn(754,v.x+60,v.y+70,player.section)
					fire2.speedY = 2
					fire2.speedX = 4
				end
			end
end
		if data.ftimer == 20 then
			v:transform(753)
		end
	end
